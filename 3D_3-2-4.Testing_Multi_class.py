...
def model_test(use_existing):
  ...
  # Lidia CTs:
  # originFile_list = sorted(glob(cm.workingPath.originLidiaTestingSet_path + "vol*.dcm"))
  # maskAortaFile_list = sorted(glob(cm.workingPath.originLidiaTestingSet_path + "vol*.dcm"))
  # maskPulFile_list = sorted(glob(cm.workingPath.originLidiaTestingSet_path + "vol*.dcm"))
  ...
  for i in range(len(originFile_list)):
    ...
    for i in range(num_patches):
      ...
      if i == int(num_patches*0.63):
        vs.visualize_activation_in_layer_one_plot(model, test_image, current_dir)
      else:
        pass
