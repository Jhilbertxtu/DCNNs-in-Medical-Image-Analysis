...
for nb_file in range(len(originFile_list)):
  ...
  for i in range(num_out):
    ...
    for j in range(int(cm.nb_batch_size)):
      out1[j] = final_images_crop_save_last[i*cm.nb_batch_size+j]
      out2[j] = final_masks_crop_save_last[i*cm.nb_batch_size+j]

    np.save(cm.workingPath.trainingPatchesSet_path + 'img_%04d.npy' %(i + pre_patch), out1)
    np.save(cm.workingPath.trainingPatchesSet_path + 'mask_%04d.npy' %(i + pre_patch), out2)
