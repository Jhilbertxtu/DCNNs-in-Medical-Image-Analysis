...
def model_test(use_existing):
  ...
  for i in range(len(originFile_list)):
    ...
    for i in range(originTestVol.shape[2]):
      img = originTestVol[-1-col:-1, row_1:row_2, i]
      out_test_images.append(img)
    for i in range(maskTestVol.shape[2]):
      img = maskTestVol[-1-col:-1, row_1:row_2, i]
      out_test_masks.append(img)

    vol_slices.append(originTestVol.shape[2])
  num_test_images = sum(vol_slices)
  final_test_images = np.ndarray([num_test_images, col, row], dtype=np.int16)
  final_test_masks = np.ndarray([num_test_images, col, row], dtype=np.int8)
  ...
  test_image = np.ndarray([1, slices, col, row, 1], dtype=np.int16)
  test_mask = np.ndarray([1, slices, col, row, 1], dtype=np.int8)

  predicted_mask_volume = np.ndarray([num_test_images, col, row], dtype=np.float32)
  ...
  for i in range(0, col):
    for j in range(0, row):
      for k in range(0,512):
        final_test_predicted_threshold[-1-i, row_1+j, k] = imgs_predict_threshold[k, -1-i, j]
