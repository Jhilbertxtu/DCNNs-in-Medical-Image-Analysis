...
for i in range(len(originFile_list)):
  ...
  final_images = np.ndarray([(vol_slices), 512, 512], dtype=np.int16)
  final_masks = np.ndarray([(vol_slices), 512, 512], dtype=np.int8)

  for j in range(len(out_images)):
    final_images[j] = out_images[j]
    final_masks[j] = out_masks[j]
  ...
  final_masks_crop_save = np.ndarray([num1, row, col, 1], dtype=np.int8)

  print('Images to Patches %04d' % (i))
  print('-' * 30)

  count = 0
  for num_vol in range(0, num_file):
    num_patches = int((vol_slices - slices) / gaps)
    if num_vol > 0:
      count = count + vol_slices
    else:
      pass
  ...
  final_masks_crop_save_last = np.ndarray([tubes, slices, row, col, 1], dtype=np.int8)

  for t in range(tubes):
    final_images_crop_save_last[t,:,:,:,:] = final_images_crop_save[t*slices:(t+1)*slices, :,:,:]
    final_masks_crop_save_last[t,:,:,:,:] = final_masks_crop_save[t*slices:(t+1)*slices, :,:,:]
  ...
  print('Saving Images ...%04d' % (i))
  print('-' * 30)

  np.save(cm.workingPath.training3DSet_path + 'trainImages_%04d.npy' % (i), final_images_crop_save_last[rand_i[:]])
  np.save(cm.workingPath.training3DSet_path + 'trainMasks_%04d.npy' % (i), final_masks_crop_save_last[rand_i[:]])
