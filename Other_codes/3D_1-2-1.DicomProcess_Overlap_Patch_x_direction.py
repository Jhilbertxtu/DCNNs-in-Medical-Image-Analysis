...
for i in range(len(originFile_list)):
  ...
  for i in range(originVol.shape[1]):
    img = originVol[-1-row:-1, i, col_1:col_2]
    # new_img = resize(img, [512, 512])
    out_images.append(img)
  for i in range(maskVol.shape[1]):
    # new_mask = resize(img, [512, 512])
    img = maskVol[-1-row:-1, i, col_1:col_2]
    out_masks.append(img)

  vol_slices.append(originVol.shape[1])

final_images = np.ndarray([sum(vol_slices), row, col], dtype=np.int16)
final_masks = np.ndarray([sum(vol_slices), row, col], dtype=np.int8)
