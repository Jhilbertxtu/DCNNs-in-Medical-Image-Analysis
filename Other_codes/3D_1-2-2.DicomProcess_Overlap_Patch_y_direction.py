...
for i in range(len(originFile_list)):
  ...
  for i in range(originVol.shape[2]):
    img = originVol[-1-col:-1, row_1:row_2, i]
    # new_img = resize(img, [512, 512])
    out_images.append(img)
  for i in range(maskVol.shape[2]):
    # new_mask = resize(img, [512, 512])
    img = maskVol[-1-col:-1, row_1:row_2, i]
    out_masks.append(img)

  vol_slices.append(originVol.shape[2])

final_images = np.ndarray([sum(vol_slices), col, row], dtype=np.int16)
final_masks = np.ndarray([sum(vol_slices), col, row], dtype=np.int8)
...
final_images_crop_save = np.ndarray([num1, col, row, 1], dtype=np.int16)
final_masks_crop_save = np.ndarray([num1, col, row, 1], dtype=np.int8)
    ...
    for num_row in range(0, num_rowes):
      for num_col in range(0, num_coles):
        ...
        final_images_crop_save = np.concatenate(
          (final_images_crop_save, final_images[count1:count2,
                                   num_col * col:(num_col * col + col), num_row * row:(num_row * row + row), :]), axis=0)

        final_masks_crop_save = np.concatenate(
          (final_masks_crop_save, final_masks[count1:count2,
                                  num_col * col:(num_col * col + col), num_row * row:(num_row * row + row), :]), axis=0)

tubes = int(final_images_crop_save.shape[0]/slices)
final_images_crop_save_last = np.ndarray([tubes, slices, col, row, 1], dtype=np.int16)
final_masks_crop_save_last = np.ndarray([tubes, slices, col, row, 1], dtype=np.int8)
...
np.save(cm.workingPath.training3DSet_path + 'trainImages3D46911.npy', final_images_crop_save_last[rand_i[:]])
np.save(cm.workingPath.training3DSet_path + 'trainMasks3D46911.npy', final_masks_crop_save_last[rand_i[:]])
