...
for i in range(len(originFile_list)):
  ...
  for j in range(len(maskVol)):
    maskVol[j] = np.where(maskVol[j] != 0, 2, 0)
...
np.save(cm.workingPath.training3DSet_path + 'trainImages3D.npy', final_images_crop_save_last[rand_i[:]])
np.save(cm.workingPath.training3DSet_path + 'trainMasks3D.npy', final_masks_crop_save_last[rand_i[:]])
